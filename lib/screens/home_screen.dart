  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, settingsSnap) {
        ...
        return Scaffold(
          appBar: AppBar(title: Text('ExamHook', style: GoogleFonts.poppins(fontWeight: FontWeight.bold)), backgroundColor: primaryGreen, elevation: 0, actions: [IconButton(icon: const Icon(Icons.mail_outline), onPressed: _showRequestDialog)]),
          drawer: Drawer(
            child: Column(
              children: [
                DrawerHeader(
                  ...
                  child: Column(
                    ...
                    children: [
                      // WRAPPED ICON IN GESTUREDETECTOR FOR 5 TAPS
                      GestureDetector(
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return ListView.builder(itemCount: docs.length, itemBuilder: (context, index) {
                      ...
                      return Card(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: primaryGreen.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.picture_as_pdf, color: primaryGreen, size: 28)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(data['title']?? 'No Title', style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 16)), Text('${data['course']} • ${data['examType']}', style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey.shade600))])), IconButton(icon: const Icon(Icons.download_for_offline, color: secondaryBlue, size: 30), onPressed: () => _openLink(docId, data['fileUrl']))]),
                        const SizedBox(height: 8), Row(children: [Icon(Icons.calendar_today, size: 12, color: Colors.grey), const SizedBox(width: 4), Text(_formatDate(data['uploadedAt']), style: GoogleFonts.poppins(fontSize: 11)), const SizedBox(width: 12), Icon(Icons.data_object, size: 12, color: Colors.grey), const SizedBox(width: 4), Text(_formatBytes(data['fileSize']?? 0), style: GoogleFonts.poppins(fontSize: 11))]),
                        const SizedBox(height: 8), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [IconButton(icon: Icon(Icons.favorite, color: isLiked? Colors.red : Colors.grey, size: 20), onPressed: () => _likeResource(docId)), Text('${data['likes']?? 0}', style: GoogleFonts.poppins(fontSize: 12))]), Row(children: [Icon(Icons.star, color: Colors.amber, size: 20), Text(' ${data['rating']?.toStringAsFixed(1)?? '0.0'}', style: GoogleFonts.poppins(fontSize: 12)), PopupMenuButton<double>(enabled:!isRated, onSelected: (val) => _rateResource(docId, val), itemBuilder: (context) => [1,2,3,4,5].map((e) => PopupMenuItem(value: e.toDouble(), child: Text('$e Star'))).toList(), child: Icon(Icons.rate_review, size: 20, color: isRated? Colors.grey : Colors.black))]), Row(children: [Icon(Icons.download, color: Colors.blue, size: 20), Text(' ${data['downloads']?? 0}', style: GoogleFonts.poppins(fontSize: 12))])])
                      ])));
                    });
                  },
                ),
